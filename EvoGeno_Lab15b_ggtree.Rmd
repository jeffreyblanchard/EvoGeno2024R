---
title: "Lab 16 : Visualizing NEON data on your phylogenetic trees"
output:
  html_document:
    toc: true
    toc_depth: 4
...
If you are working Bacteria you first need to find the internal node on the tree corresponding to your Domain, Phylum or Class. Below is an expample 
```{r}
...
If you are working with Archaea do not need to get a subtree. You can identify the Phyla and color as below
...
ggtree(tree_bac, layout="circular", branch.length="none") +
    ...
    geom_hilight(node=1712, fill="steelblue", alpha=.6) +
    geom_cladelab(node=1712, label="Chloroflexota", align=TRUE, hjust=-0.1, offset = 0, textcolor='steelblue', barcolor='steelblue') +
    ...
    geom_cladelab(node=3047, label="Pseudomonadota ", align=TRUE, hjust=-0.1, offset = 0, textcolor='purple', barcolor='purple')
```
...
ggtree(tree_bac, layout="circular", branch.length="none") %>% 
  collapse(node=1712) + 
  geom_point2(aes(subset=(node==1712)), shape=23, size=5, fill='steelblue') +
  geom_cladelab(node=1712, label="Chloroflexota", align=TRUE,  
...
### Filter to your site MAGs

Filter to either Bacteria or Archaea
```{r}
...
Now make a tree with your site MAGs
...
### A tree with a particular ecosystem subtype


```{r, fig.height = 12, fig.width = 12}
# Filter to either Bacteria or Archaea and parameter of interest
NEON_MAGs_metagenomes_chemistry_soilInCaClpH <- NEON_MAGs_metagenomes_chemistry %>% 
  filter(soilInCaClpH > 7) %>% 
  filter(Domain == "Bacteria")
# Make a vector of the MAGs labels
